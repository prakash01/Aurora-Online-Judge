...
function display_submissions(){
	...
	if(isset($_GET["pid"]) && !empty($_GET["pid"]) && is_numeric($_GET["pid"])){
		$t = mysql_query("SELECT * FROM problems WHERE pid=".$_GET["pid"]." and status='Active'");
		if(is_resource($t) && mysql_num_rows($t)){
			$filter["pid"] = $_GET["pid"];
	...
	if(isset($_GET["pgr"]) && !empty($_GET["pgr"]) && in_array($_GET["pgr"],$g)){
		$t = mysql_query("SELECT * FROM problems WHERE pgroup='".addslashes($_GET["pgr"])."' and status='Active'");
		if(is_resource($t) && mysql_num_rows($t)){
	...
	if(isset($filter["tid"])){
		...
		$data = mysql_query("SELECT distinct(runs.pid),problems.name,problems.code FROM runs,problems WHERE runs.tid='$filter[tid]' and runs.result='AC' and runs.pid=problems.pid and problems.status='Active' and runs.access!='deleted'");
		if(is_resource($data)){
	...
	if(!isset($filter["result"])){
		$t1 = mysql_query("SELECT result,count(*) as cnt FROM runs WHERE access!='deleted' AND tid in (SELECT tid FROM teams WHERE status='Normal' OR status='Admin') AND pid in (SELECT pid FROM problems WHERE status='Active') $condition group by result;");
	...
	if(!isset($filter["language"])){
		$t1 = mysql_query("SELECT language,count(*) as cnt FROM runs WHERE access!='deleted' AND tid in (SELECT tid FROM teams WHERE status='Normal' OR status='Admin') AND pid in (SELECT pid FROM problems WHERE status='Active') $condition group by language;");
	...
	$total = mysql_query("SELECT count(*) as total FROM runs WHERE access!='deleted' AND tid in (SELECT tid FROM teams WHERE status='Normal' OR status='Admin') AND pid in (SELECT pid FROM problems WHERE status='Active') $condition ORDER BY rid DESC");
	$total = mysql_fetch_array($total); $total = $total["total"];
	if(isset($admin["substatpage"]) && $admin["substatpage"]>=0) $limit=$admin["substatpage"]; else $limit=25;
	if(isset($_GET["page"]) && is_numeric($_GET["page"])) $page = max(0,$_GET["page"]); else $page = 0;
	
	$pagenav="";
	if($page>0) $pagenav.="<a href='?".$urlargs."&page=".($page-1)."'>Previous Page</a>"; else $pagenav.="Previous Page";
	$pagenav.=" : ".($page+1)."/".ceil($total/$limit)." : ";
	if(($page+1)*$limit<$total) $pagenav.="<a href='?".$urlargs."&page=".($page+1)."'>Next Page</a>"; else $pagenav.= "Next Page";
	
	echo $pagenav."<br><br>";
	echo "<table class='submission'><th>Run ID</th><th>Team</th><th>Problem</th><th>Language</th><th>Time</th><th>Result</th>".(($_SESSION["status"]=="Admin")?"<th style='width:170px;'>Administrator</th>":"")."</tr>";
	$data = mysql_query("SELECT * FROM runs WHERE access!='deleted' AND tid in (SELECT tid FROM teams WHERE status='Normal' OR status='Admin') AND pid in (SELECT pid FROM problems WHERE status='Active') $condition ORDER BY rid DESC LIMIT ".($page*$limit).",".$limit);
	...
	if(is_resource($data)) for($i=0;$temp = mysql_fetch_array($data);$i++){
		...
		$t = mysql_query("SELECT name FROM problems WHERE pid=$temp[pid] and status='Active'"); if(is_resource($t) && mysql_num_rows($t)==1){ $t = mysql_fetch_array($t); $probname=$t['name']; } else continue;
		$fresult = $result = $temp["result"]; if(isset($fullresult[$result])) $fresult = $fullresult[$result];
		if($result==""){ $result="NA"; $fresult="Queued"; } if($result=="..."){ $result="NA"; $fresult="Evaluating<img1 src='data/loading.gif'>"; }
			if($_SESSION["status"]=="Admin" || $_SESSION["tid"]==$temp["tid"] || $temp["access"]=="public")
				echo "<tr class='$result'><td><a href='?display=code&rid=$temp[rid]' title='Link to Code'>$temp[rid]</a></td><td><a href='?".str_replace("&tid=$temp[tid]","",$urlargs)."&tid=$temp[tid]' title='Link to Team'>$teamname</td><td><a href='?".str_replace("&pid=$temp[pid]","",$urlargs)."&pid=$temp[pid]' title='Link to Problem'>$probname</a></td><td><a href='?".str_replace("&lan=".urlencode($temp["language"]),"",$urlargs)."&lan=".urlencode($temp["language"])."' title='Link to $temp[lan] Submissions'>$temp[lan]</a></td><td>$temp[time]</td><td class='$result'><a href='?$urlargs&res=$result'>$fresult</a></td>";
...
function display_rankings(){
	...
	$total = mysql_query("SELECT count(*) as total FROM teams WHERE status='Normal'");
	$total = mysql_fetch_array($total); $total = $total["total"];
	if(isset($admin["rankpage"])) $limit = $admin["rankpage"]; else $limit = 25;
	if(isset($_GET["page"]) && is_numeric($_GET["page"])) $page = max(0,$_GET["page"]); else $page = 0;
	$pagenav="";
	if($page>0) $pagenav.= "<a href='?display=rankings&page=".($page-1)."'>Previous Page</a>"; else $pagenav.="Previous Page";
	$pagenav.=" : ".($page+1)."/".ceil($total/$limit)." : ";
	if(($page+1)*$limit<$total) $pagenav.= "<a href='?display=rankings&page=".($page+1)."'>Next Page</a>"; else $pagenav.="Next Page";
	...
	echo $pagenav."<br><br>";
	echo "<table><th>Rank</th><th>Team</th><th>Problems Solved / Attempted</th><th>Score</th></tr>";
	$data = mysql_query("SELECT * FROM teams WHERE status='Normal' ORDER BY score DESC, penalty ASC LIMIT ".($page*$limit).",".$limit);
	if(is_resource($data)) for($rank=1; $temp = mysql_fetch_array($data); $rank++){
		...
		echo "<tr><td>$rank</td><td><a href='?display=submissions&tid=$temp[tid]'>$temp[teamname]</a></td><td>$solvedn / $solveda</div></td><td>$temp[score]</td></tr>";
		}
