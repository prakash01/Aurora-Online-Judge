...
function action_ajaxrefresh($type){
	...
	if( ($admin["mode"]=="Lockdown" && $_SESSION["status"]!="Admin") ||!isset($admin["cache-problems"]))
	...
	if(is_resource($data) && mysql_num_rows($data)){
		...
		while($temp = mysql_fetch_array($data)){
			$prob = mysql_query("SELECT name FROM problems WHERE pid=".$temp["pid"]." and ");
			$json["newclar"][]=("Query : ".unfilter($temp["query"]).(!empty($temp["reply"])?"\nResponse : ".unfilter($temp["reply"]):""));
			}
