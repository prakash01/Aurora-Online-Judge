...
function action_register(){
	...
	$_POST["reg_ip"] = json_encode(array($_SERVER["REMOTE_ADDR"]));
	...
	foreach($_POST as $key=>$value) if($key!="reg_pass1" && $key!="reg_pass2"){ $temp1[]=eregi_replace("reg_","",$key); $temp2[]=filter($value); }
...
function action_login(){
	...
	$t = mysql_query("SELECT * FROM teams WHERE teamname='".filter($_POST["team"])."'");
