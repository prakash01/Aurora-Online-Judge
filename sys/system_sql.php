...
function mysql_initiate(){
	global $mysql_hostname,$mysql_username,$mysql_password,$mysql_database,$admin,$ajaxlogout,$sessionid;
	...
	if(!in_array("teams",$table)){ mysql_query("CREATE TABLE teams (tid int not null primary key auto_increment,teamname tinytext,pass tinytext,status tinytext,score int,penalty bigint,name1 tinytext,roll1 tinytext,branch1 tinytext,email1 tinytext,phone1 tinytext,name2 tinytext,roll2 tinytext,branch2 tinytext,email2 tinytext,phone2 tinytext,name3 tinytext,roll3 tinytext,branch3 tinytext,email3 tinytext,phone3 tinytext,platform text,ip text,session tinytext)"); }
	if(!in_array("problems",$table)){ mysql_query("CREATE TABLE problems (pid int not null primary key auto_increment,code tinytext,name tinytext,type tinytext,status tinytext,pgroup tinytext,statement longtext,image blob,imgext tinytext,input longtext,output longtext,timelimit int,score int,languages longtext)"); }
	if(!in_array("runs",$table)){ mysql_query("CREATE TABLE runs (rid int not null primary key auto_increment,pid int,tid int,language tinytext,name tinytext,code longtext,time tinytext,result tinytext,error text,access tinytext,submittime int)"); }
	...
	$temp = mysql_query("SELECT * FROM teams"); if(is_resource($temp) && mysql_num_rows($temp)==0){
		mysql_query("INSERT INTO teams (teamname,pass,status,score,name1,roll1,branch1,email1,phone1) VALUES ('".($admin_teamname)."','".md5($admin_password)."','Admin',0,'Kaustubh Karkare','BE/1184/09','ECE','kaustubh.karkare@gmail.com','9572161565')");
		}
	$temp = mysql_query("SELECT * FROM problems"); if(is_resource($temp) && mysql_num_rows($temp)==0){
		mysql_query("INSERT INTO problems (pid,code,name,type,status,statement,input,output,timelimit,score,languages) VALUES (1,'Squares','Squares','Ad-Hoc','Active','Test','".addslashes(file_get('data/example/problem.txt'))."','".addslashes(file_get('data/example/input.txt'))."','".addslashes(file_get('data/example/output.txt'))."',1,0,'Brain,C,C++,Java,JavaScript,Pascal,Perl,PHP,Python,Ruby,Text')");
		}
	$temp = mysql_query("SELECT * FROM runs"); if(is_resource($temp) && mysql_num_rows($temp)==0){
		...
		mysql_query("INSERT INTO runs (rid,pid,tid,language,name,code,time,result,access) VALUES (3,1,1,'Java','code','".(addslashes(file_get('data/example/code.java')))."',NULL,NULL,'public')");
		mysql_query("INSERT INTO runs (rid,pid,tid,language,name,code,time,result,access) VALUES (4,1,1,'JavaScript','code','".(addslashes(file_get('data/example/code.js')))."',NULL,NULL,'public')");
		mysql_query("INSERT INTO runs (rid,pid,tid,language,name,code,time,result,access) VALUES (5,1,1,'Pascal','code','".(addslashes(file_get('data/example/code.pas')))."',NULL,NULL,'public')");
		mysql_query("INSERT INTO runs (rid,pid,tid,language,name,code,time,result,access) VALUES (6,1,1,'Perl','code','".(addslashes(file_get('data/example/code.pl')))."',NULL,NULL,'public')");
		mysql_query("INSERT INTO runs (rid,pid,tid,language,name,code,time,result,access) VALUES (7,1,1,'PHP','code','".(addslashes(file_get('data/example/code.php')))."',NULL,NULL,'public')");
		mysql_query("INSERT INTO runs (rid,pid,tid,language,name,code,time,result,access) VALUES (8,1,1,'Python','code','".(addslashes(file_get('data/example/code.py')))."',NULL,NULL,'public')");
		mysql_query("INSERT INTO runs (rid,pid,tid,language,name,code,time,result,access) VALUES (9,1,1,'Ruby','code','".(addslashes(file_get('data/example/code.rb')))."',NULL,NULL,'public')");
		}
