...
//for($i=0;$i<256;$i++) echo $i." = ".chr($i)."<br>";
//foreach($_SERVER as $key=>$value) echo "$key => $value<br>";
//foreach(folder_get("sys") as $file) if(eregi("^system",$file)) if(eregi("ajaxlogout",file_get("sys/$file"))) echo $file."<br>";
?>
...
function process(key,value){
	...
	else if(key=="newclar"&&value!="") alert(value);
...
function init(){
	...
	$("code").each( function(index){
		$(this).html( "<br><div class='limit code'>"+$(this).html().replace(/<br>/,'')+"</div>" );
...
<center><h1>Aurora Online Judge [SourceCode]</h1></center>
<center><table class='main'><tr><td class='side'>
	...
	<div class='sidebox'>
		...
		<?php echo "<li><a href='?display=scoreboard'>Main Scoreboard</a></li>"; ?>
		...
		<?php if($_SESSION["status"]=="Admin"){
			...
			echo "<li><a href='?display=admindata'>Data Commitment</a></li>";
	...
	<div class='sidebox'><i>Created by Kaustubh Karkare [<a href='http://192.168.154.51/'>SourceCode</a>].</i></div>
</td></table></center>
<a name="bottom"></a>
